...
class MCPServer:
    ...
    def __init__(self, hass: HomeAssistant, entry: Any) -> None:
        ...
        self.mcp_server = Server(MCP_SERVER_NAME)
        self._setup_mcp_tools()
        self.sessions = {}  # Track SDK sessions by connection ID or session ID
    ...
    async def handle_websocket(self, request: web.Request) -> web.WebSocketResponse:
        """Handle incoming WebSocket connections and bridge them to the MCP SDK."""
        # IP filter check
        client_ip = request.remote
        if not self._is_ip_allowed(client_ip):
            _LOGGER.warning("Access denied to MCP WebSocket from %s", client_ip)
            return web.Response(status=403, text="Forbidden")

        ws = web.WebSocketResponse()
        await ws.prepare(request)
        
        # Create anyio streams for the SDK to read from and write to
        ...
        _LOGGER.info("MCP WebSocket connection established from %s", client_ip)

        async def ws_to_sdk_task():
            """Task to pipe WebSocket messages into the SDK's read stream."""
            try:
                async with read_stream_writer:
                    async for msg in ws:
                        if msg.type == web.WSMsgType.TEXT:
                            try:
                                client_message = types.JSONRPCMessage.model_validate_json(msg.data)
                                await read_stream_writer.send(SessionMessage(client_message))
                            except Exception as exc:
                                _LOGGER.error("Failed to parse WebSocket message: %s", exc)
                        elif msg.type == web.WSMsgType.ERROR:
                            break
            except (anyio.ClosedResourceError, asyncio.CancelledError):
                pass
            finally:
                _LOGGER.debug("ws_to_sdk_task finished")

        async def sdk_to_ws_task():
            """Task to pipe SDK's write stream back into the WebSocket."""
            try:
        ...
        async def run_sdk_session():
            try:
                ...
                _LOGGER.error("Error in SDK session: %s", e)

        # Use Home Assistant's async_create_task to run the tasks since we're in a handler
        self.hass.async_create_task(ws_to_sdk_task())
        self.hass.async_create_task(sdk_to_ws_task())
        self.hass.async_create_task(run_sdk_session())

        return ws

    async def handle_mcp_request(self, request: web.Request) -> web.Response:
        """Handle HTTP MCP POST requests by bridging to a short-lived SDK session."""
        client_ip = request.remote
        if not self._is_ip_allowed(client_ip):
        ...
        try:
            ...
            client_message = types.JSONRPCMessage.model_validate_json(body)
            
            # For POST requests, we simulate a single-message session
            ...
            async def run_once():
                async with read_stream_writer:
                    await read_stream_writer.send(SessionMessage(client_message))
                init_options = self.mcp_server.create_initialization_options()
                # Run the server for just this one message (stateless=True if supported)
                await self.mcp_server.run(read_stream, write_stream, init_options, raise_exceptions=True)

            self.hass.async_create_task(run_once())
            
            # Wait for the response from the SDK
            async with write_stream_reader:
                async for session_message in write_stream_reader:
                    # We expect at least one response (e.g. initialize if first, or the result)
                    return web.json_response(session_message.message.model_dump(by_alias=True, exclude_none=True))

            return web.json_response({"error": "No response from SDK"}, status=500)
        except Exception as e:
            return web.json_response({"error": str(e)}, status=400)

    async def handle_mcp_sse(self, request: web.Request) -> web.StreamResponse:
        """Handle MCP SSE connections by bridging to a persistent SDK session."""
        # This is complex because we need to link the SSE 'write' stream
        # with subsequent POST 'read' messages.
        # For now, let's stick to the official tool calling in agent.py
        # and support deep SDK usage mostly in WebSockets which is what Cursor/others use.
        return web.Response(text="SSE bridge not implemented", status=501)
    ...
    async def handle_tool_call(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        _LOGGER.info("Executing tool: %s with arguments: %s", name, arguments)

        try:
            if name == "discover_entities":
                result = await self.discovery.discover_entities(**arguments)
            elif name == "get_entity_details":
                entity_ids = arguments.get("entity_ids", [])
                result = await self.discovery.get_entity_details(entity_ids)
            elif name == "perform_action":
                domain = arguments.get("domain")
                action = arguments.get("action")
                target = arguments.get("target")
                data = arguments.get("data")
                result = await self.discovery.perform_action(domain, action, target, data)
            elif name == "get_index":
                from .__init__ import get_index_manager
                index_manager = get_index_manager(self.hass)
                result = index_manager.get_index()
            else:
    ...
    async def process_mcp_message(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """Process MCP message using the official SDK's protocol engine."""
        try:
            # Create a simple transport adapter for the SDK
            # Since we are already in an async handler, we can use the server's internal dispatching
            # However, the SDK expects a transport. For now, let's manualy call the handlers
            # to avoid refactoring the entire aiohttp loop into a transport yet.

            method = data.get("method")
            params = data.get("params", {})
            msg_id = data.get("id")

            _LOGGER.debug("Processing MCP method via SDK: %s (id: %s)", method, msg_id)

            # Map JSON-RPC to SDK calls
            if method == "initialize":
                # The SDK usually handles this via the transport, but we can call it manually
                return {
                    "jsonrpc": "2.0",
                    "id": msg_id,
                    "result": {
    ...
    def _setup_mcp_tools(self):
        """Register tools with the official SDK server instance."""
            ...
            return [
                types.Tool(
                    ...
                    inputSchema={
                        "type": "object",
                        "properties": {
                            "entity_type": {"type": "string", "description": "Type of entity to find (e.g., 'light', 'switch', 'sensor', 'climate')"},
                            "area": {"type": "string", "description": "Area/room name to search in"},
                            "domain": {"type": "string", "description": "Home Assistant domain to filter by"},
                            "state": {"type": "string", "description": "Current state to filter by"},
                            "name_contains": {"type": "string", "description": "Text that entity name should contain"},
                            "device_class": {
                                "oneOf": [{"type": "string"}, {"type": "array", "items": {"type": "string"}}],
                                "description": "Device class to filter by",
                            },
                            "limit": {"type": "integer", "description": f"Max entities (max: {max_limit})", "default": 20},
                        },
                    },
                ),
                types.Tool(
                ...
                types.Tool(
                    ...
                    inputSchema={
                        "type": "object",
                        "properties": {
                            "domain": {"type": "string", "description": "The domain of the service"},
                            "action": {"type": "string", "description": "The service action"},
                            "target": {"type": "object", "description": "Target entities, areas, or devices"},
                ...
                types.Tool(
                    name="get_index",
                    description="Get the pre-generated system structure index.",
        ...
        async def handle_call_tool(name: str, arguments: dict | None) -> list[types.TextContent | types.ImageContent | types.EmbeddedResource]:
            ...
            try:
                result = await self.handle_tool_call({"name": name, "arguments": arguments or {}})
                # Map our internal format to SDK format
                content = []
                if isinstance(result, dict) and "content" in result:
                    ...
                    content.append(types.TextContent(type="text", text=json.dumps(result, default=str)))
